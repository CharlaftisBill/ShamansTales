...
class AudioManager {
    ...
    playSyntheticSFX(name, pitchVariance) {
        ...
        if (name === 'select') {
            ...
            // Hologram reactivation / response to summon
            osc.type = 'triangle';
            osc.frequency.setValueAtTime(600 * variance * stressMult, now); // Starts where summon ended
            osc.frequency.exponentialRampToValueAtTime(1200 * variance * stressMult, now + 0.15); // Sweeps up

            // Shared LFO DNA for that hologram connection
            const lfo = this.ctx.createOscillator();
            lfo.type = 'sine';
            lfo.frequency.value = 35; // Slightly faster pulse to signify "ready"
            ...
            gain.gain.setValueAtTime(0, now);
            gain.gain.linearRampToValueAtTime(0.15 * volMult, now + 0.05);
