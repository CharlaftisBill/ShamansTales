...
async function preloadDeck(factionName, deck) {
    ...
    const imageUrls = deck.map(card => `assets/icons/cards/${factionName}/${card.id}.png`);
...
async function renderDeck(factionName) {
    ...
    deck.forEach(card => {
        ...
        cardWrapper.innerHTML = `
            <img class="full-art-image" src="assets/icons/cards/${factionName}/${card.id}.png" onerror="this.src=''" alt="${card.name}">
