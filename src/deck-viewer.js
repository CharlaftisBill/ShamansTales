...
async function preloadDeck(factionName, deck) {
    ...
    const imageUrls = deck.map(card => `../assets/hq/icons/cards/${factionName.toLowerCase()}/${card.id}.png`);
...
async function renderDeck(factionName) {
    ...
    deck.forEach(card => {
        ...
        cardWrapper.innerHTML = `
            <img class="full-art-image" src="../assets/hq/icons/cards/${factionName.toLowerCase()}/${card.id}.png" onerror="this.src=''" alt="${card.name}">
            <div class="full-art-gradient-top"></div>
            <div class="full-art-gradient-bottom"></div>

            <div class="full-art-cost-container">
                <img class="full-art-cost-icon" src="../assets/hq/icons/ui/mechanics/cost_emblem.png" alt="Cost">
                <span class="full-art-cost-value">${card.cost}</span>
            </div>

            <div class="full-art-class-container">
                <div style="position: relative;">
                    <img class="full-art-class-icon" src="../assets/hq/icons/ui/classes/${card.fightingClass.toLowerCase()}_emblem.png" onerror="this.style.display='none'" alt="${card.fightingClass}">
...
function updatePreviewPane(card, factionName) {
    ...
    const premiumHTML = `
        <div class="premium-card-25d friendly">
            <img class="full-art-image glitch-reveal" src="../assets/hq/icons/cards/${factionName.toLowerCase()}/${card.id}.png" onerror="this.src=''" alt="${card.name}">
            <div class="full-art-gradient-top"></div>
            <div class="full-art-gradient-bottom"></div>

            <div class="full-art-cost-container inspect-scale">
                <img class="full-art-cost-icon" src="../assets/hq/icons/ui/mechanics/cost_emblem.png" alt="Cost Icon">
                <span class="full-art-cost-value">${card.cost}</span>
            </div>

            <div class="full-art-class-container inspect-scale">
                <div style="position: relative;">
                    <img class="full-art-class-icon" src="../assets/hq/icons/ui/classes/${card.fightingClass.toLowerCase()}_emblem.png" onerror="this.style.display='none'" alt="Class">
                    <div class="full-art-amp-value">${card.fcAmplifier}</div>
                </div>
            </div>
